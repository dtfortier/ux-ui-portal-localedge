...
function bootstrapcanvaswp_setup() {
	...
	register_nav_menus( array(
        ...
        'style-guide-sub-menu' =>__('Style Guide Sub Menu'),
        'dropdown-sub-menu' =>__('Dropdown Sub Menu'),
	) );
...
function bootstrapcanvaswp_scripts() {
    ...
    wp_enqueue_style( 'prototypes-css', get_template_directory_uri() . '/css/prototypes.css' );
    wp_enqueue_style( 'custom-css', get_template_directory_uri() . '/css/custom.css' );
    wp_enqueue_style( 'le-dashboard-css', get_template_directory_uri() . '/css/le-dashboard.css' );
    wp_enqueue_style( 'ui-standards-css', get_template_directory_uri() . '/css/ui-standards.css' );
	wp_enqueue_style( 'bootstrap-css', get_template_directory_uri() . '/css/bootstrap.css', '3.3.0' );
    wp_enqueue_style( 'formValidation-css', get_template_directory_uri() . '/css/bootstrapValidator.css', '3.3.0' );
	if ( is_rtl() ) {
    ...
    wp_enqueue_script( 'scripts-js', get_template_directory_uri() . '/js/jquery-1.11.3.min.js', array( 'jquery' ), '', true );
    wp_enqueue_script( 'bootstrap-validator-js', get_template_directory_uri() . '/js/bootstrapValidator.js', array( 'jquery' ), '', true );
...
$counter = 1;
$string .= '<ul>';
while( $lastupdated_loop->have_posts() && $counter < 10 ) : $lastupdated_loop->the_post();
$string .= '<li><a href="' 
    . get_permalink( $lastupdated_loop->post->ID ) 
    . '"> ' .get_the_title( $lastupdated_loop->post->ID ) 
    . '</a>  <div class="author">' . get_the_author() 
    .' added or modified a ' . get_post_type() 
    . ' </div> <div class="date">'. get_the_modified_date() .  ', ' . get_the_modified_time() . '</div> ' . get_the_category_list($parents, $post_id ) 
    .'  </li>';
$counter++;
endwhile;    
$string .= '</ul>';
...
function wpb_stndrdUpdates_posts() { 

// Query Arguments
$statusupdated_args = array(
'orderby' => 'created',
'ignore_sticky_posts' => '1',
'post_type' => array('standards update')
);

//Loop to display 8 recently updated posts
$statusupdated_loop = new WP_Query( $statusupdated_args );
$counter = 1;
$string .= '<ul>';
while( $statusupdated_loop->have_posts() && $counter < 10 ) : $statusupdated_loop->the_post();
$string .= '<li><a href="' 
    . get_permalink( $statusupdated_loop->post->ID ) 
    . '"> ' .get_the_title( $statusupdated_loop->post->ID ) 
    . '<div class="version">   v.'
    . get_field('version') . '</a> </div> <div class="date">'
    . get_the_date() .  ', ' 
    . get_the_time() . '</div> '
    . '<div class="details">'
    . get_the_content() . '</div>'
    . '<div class="details">'
    . get_field('details') . '</div>'
    .'  </li>' ;
$counter++;
endwhile;    
$string .= '</ul>';
...
//add a shortcode
add_shortcode('statusupdated-posts', 'wpb_stndrdUpdates_posts');
...
// Adding RSS Feed for Standards updates  //

add_action('init', 'customRSS');
function customRSS(){
        add_feed('standardsupdates', 'customRSSFunc');
}


// Adding Custom Posts to Category Archive  //

 function show_cpt_archives( $query ) {
    if( is_category() || is_tag() && empty( $query->query_vars['suppress_filters'] ) ) {
    $query->set( 'post_type', 
                array('post', 'nav_menu_item', 'mockup')
               );
 return $query;
 }
}
